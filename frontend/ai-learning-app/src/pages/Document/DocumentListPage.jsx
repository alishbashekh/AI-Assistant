...
const fetchDocuments = async () => {
  try {
    ...
    console.log("First doc _id:", data?.[0]?._id);
    setDocuments(data); // ✅ ADD THIS BACK
  } catch(error) {
        ...
        <div className="relative w-full max-w-lg bg-white/95 backdrop-blur-xl border border-slate-200/60 rounded-2xl shadow-2xl shadow-slate-900/20 p-8">
          ...
          <form onSubmit={handleUpload} className='space-y-5'>
            ...
            <div className="space-y-2">
              <label className='block text-xs font-semibold text-slate-700 uppercase tarcking-wide'>
                Document Title
              </label>
              <input
              type='text'
            ...
            <div className="flex gap-3 pt-2">
              ...
              type='submit'
              disabled={uploading}
              className='flex-1 h-11 px-4 bg-linear-to-r from-emerald-500 to-teal-500 hover:from-emerald-600 hover:to-teal-600 text-white text-sm font-semibold rounded-xl transition-all duration-200 '
              >
                {uploading ? (
      ...
      {isDeleteModalOpen && (<div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/50 backdrop-blur-sm">
        <div className="relative w-full max-w-md bg-white/95 backdrop-blur-xl border border-slate-200/60 rounded-2xl shadow-2xl shadow-slate-900/20 p-8">
          ...
          <button
           ...
           className='absolute top-6 right-6 w-8 h-8 flex items-center justify-center rounded-lg text-slate-400 transition-all duration-200'
          >
            <X className='w-5 h-5' strokeWidth={2}/>
          </button>
          {/*modal header */}
          <div className="mb-6">
            <div className="w-12 h-12 rounded-xl bg-linear-to-r from-red-100 to-red-200 items-center justify-center mb-4">
          ...
          <p className='text-sm text-slate-600 mb-6'>
            Are you sure you want to delete the docment:{" "}
          ...
          <div className='flex gap-3'>
            ...
            disabled={deleting}
            className='flex-1 h-11 px-4 border-2 border-red-200 rounded-xl bg-red-500 text-slate-700 text-sm font-semibold hover:bg-slate-50 hover:border-slate-300 transition-all duration-200 disabled:opacity-50'
            >
              {deleting ? (
                <span className='flex items-center justify-center gap-2'>
